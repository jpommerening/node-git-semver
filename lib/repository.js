...
Repository.prototype.config = function (cb) {
  return passThroughGitdir(this, config({local: true}, cb));
};

Repository.prototype.HEAD = function (cb) {
  return passThroughGitdir(this, reference('HEAD', cb));
};
...
Repository.prototype.checkout = function (/* [reference], [options], [cb] */) {
  ...
  if (this.gitdir) {
    cmd.emit('gitdir', this.gitdir);
  } else {
    this.on('gitdir', cmd.emit.bind(cmd, 'gitdir'));
  }
  if (this.worktree) {
    cmd.emit('worktree', this.worktree);
  } else {
    this.on('worktree', cmd.emit.bind(cmd, 'worktree'));
  }
