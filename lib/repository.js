...
Repository.prototype.config = function (cb) {
  var cfg = config({ gitdir: this.gitdir, worktree: this.worktree }, cb);
  var repo = this;
  this.ready(function () {
    cfg.emit('gitdir', repo.gitdir);
  });
  return cfg;
};

Repository.prototype.tags = function (cb) {
  var tags = refs({ path: 'refs/tags', gitdir: this.gitdir, worktree: this.worktree }, cb);
  var repo = this;
  this.ready(function () {
    tags.emit('gitdir', repo.gitdir);
  });
  return tags;
};

Repository.prototype.heads = function (cb) {
  var heads = refs({ path: 'refs/heads', gitdir: this.gitdir, worktree: this.worktree }, cb);
  var repo = this;
  this.ready(function () {
    heads.emit('gitdir', repo.gitdir);
  });
  return heads;
};
